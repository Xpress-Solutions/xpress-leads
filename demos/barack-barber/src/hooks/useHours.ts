...
export function getHoursStatus(): HoursStatus {
  ...
  if (upcoming) {
    const isBreak = today.slots?.some((slot) => now.minutes >= slot.endMin) ?? false;
    return {
      weekday: now.weekday,
      label: today.label,
      open: false,
      headline: isBreak ? "Intervalo" : "Abre hoje",
      detail: isBreak
        ? `volta às ${formatUntil(upcoming.startMin)}`
        : `às ${formatUntil(upcoming.startMin)}`,
    };
  }
  ...
  return {
    ...
    detail: today.slots ? "volta no próximo expediente" : "abre segunda às 9h",
  };
}
