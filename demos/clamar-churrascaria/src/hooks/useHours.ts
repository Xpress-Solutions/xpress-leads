...
export function getHoursStatus(): HoursStatus {
  ...
  if (current) {
    const isLunch = current.startMin < 16 * 60;
    return {
      weekday: now.weekday,
      label: today.label,
      open: true,
      headline: isLunch ? "Almoço aberto" : "Jantar aberto",
  ...
  if (upcoming) {
    const isLunch = upcoming.startMin < 16 * 60;
    return {
      weekday: now.weekday,
      label: today.label,
      open: false,
      headline: isLunch ? "Almoço hoje" : "Jantar hoje",
      detail: `abre às ${formatUntil(upcoming.startMin)}`,
    };
  }
  ...
  return {
    ...
    detail: today.slots ? "volta no próximo expediente" : "domingo a casa descansa",
  };
}
